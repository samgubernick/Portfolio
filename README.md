# Portfolio

*C++, Android NDK, SDL2 (later SDL3), OpenGL, Bullet3*

Video capture of an early prototype platform game I made using OpenGL:

![Early platform game](https://github.com/user-attachments/assets/5ad87fc7-3881-42e7-8056-a1640154e2e1)

Turned into this using Bullet to simplify the 3D calculations and movement:
![Early platform game](3d-platform-game.png)

Rough menu for functionality, which also allows custom themes through JSON:
![Early platform game](skinned-menu.png)
